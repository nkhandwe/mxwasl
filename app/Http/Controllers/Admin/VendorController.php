...
class VendorController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        if ($request->type == 'excel') {
            return (new FastExcel(Helpers::export_stores_item($stores)))->download('Stores.xlsx');
        } elseif ($request->type == 'csv') {
            return (new FastExcel(Helpers::export_stores_item($stores)))->download('Stores.csv');
        }
    }
