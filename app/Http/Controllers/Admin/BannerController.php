...
class BannerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('image')) {
            ...
            $file->move('storage/app/public/banner', $filename);
           $banner->image = $filename;
        }
        // $banner->image = Helpers::upload('banner/', 'png', $request->file('image'));
        // $banner->image = Helpers::upload('banner/', $request->file('image'));
        // $banner->video = Helpers::upload('banner/', $request->file('video'));
        ...
        $banner->zone_id = $request->zone_id;
         if ($request->file('image')) {
           ...
           $banner->image = $filename;
            ...
            // $banner->image = $request->has('image') ? Helpers::update('banner/', $banner->image, 'video', $request->file('image')) : $banner->image;
        
        
        // $banner->video = $request->has('video') ? Helpers::update('banner/', $banner->video, 'video', $request->file('video')) : $banner->video;
        $banner->data = ($request->banner_type == 'store_wise')?$request->store_id:(($request->banner_type == 'item_wise')?$request->item_id:'');
        $banner->default_link = $request->default_link;
        $banner->save();
