...
class OrderController extends Controller
{
    ...
    public function details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            ...
            $deliveryData=DeliveryMan::where('status',1)->get();
            return view('admin-views.order.order-view', compact('order', 'deliveryMen', 'deliveryData','categories', 'products', 'category', 'keyword', 'editing'));
        } else {
            Toastr::info(translate('messages.no_more_orders'));
            return back();
        }
    }
    public static function get_on_the_way($id){
    	$orders=Order::where('delivery_man_id',$id)->where('order_status','picked_up')->get();
    	return count($orders);
    }
     public static function get_handover($id){
    	$orders=Order::where('delivery_man_id',$id)->where('order_status','handover')->get();
    	return count($orders);
    }
